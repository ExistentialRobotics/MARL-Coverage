...
class SensGridRL(object):
    ...
    def __init__(self, gridlis, env_config, use_graph=False):
        ...
        self._sensorAbs = env_config['sensor_abstract']

        if self._sensorAbs == "LIDAR":
            self._sensor = LidarSensor(10, self._egoradius)
        elif self._sensorAbs == "Grid":
            self._sensor = Grid(self._egoradius)
        else:
            print("ERROR: sensor class improperly declared in config...defaulting to LIDAR")
            self._sensor = LidarSensor(10, self._egoradius)
    ...
    def get_egocentric_observations(self):
        ...
        for i in range(self._numrobot):
            ...
            z[i][2] = self._sensor.getMeasurement
            z[i][1] = self.arraySubset(self._free_pad[i], x, y, self._egoradius)
            z[i][0] = self.arraySubset(self._robot_pad, x, y, self._egoradius)
