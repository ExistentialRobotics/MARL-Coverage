...
class ErgodicController(Controller):
    ...
    def __init__(self, numrobot, xcoor, res, numbasis, dt, coverage_dist=None,  klis=None, umax=5, avoidobstacles=False):
        ...
        if(self._klis == None):
            #changing numbasis to smallest square number less than it
            rt = int(math.sqrt(numbasis))
            self._numbasis = rt**2
            self._klis = []
            for i in range(rt):
                for j in range(rt):
                    self._klis.append(np.array([[i], [j]]))
    ...
    def getControls(self, observation):
        ...
        #computing phi (metric of ergodicity): equation 2
        s_k = S_k * 1.0/(self._numrobot*self._totaltime)
        phi = 0
        for i in range(self._numbasis):
            phi += self.calculateLambda(self._klis[i]) * ((s_k[i][0])**2)
    ...
    def obstacleField(self, x, obstacle, p):
        ...
        if(np.transpose(p) @ r >= 0):
            l = 1
        res = l*(np.transpose(p) @ r)[0][0] * r - p * (np.transpose(r) @ r)[0][0]
        return res
    ...
    def computehk(self, k):
        """
        computes the normalization constant for a fourier basis function given the
        wavenumber, the normalization makes the function have magnitude
        """
