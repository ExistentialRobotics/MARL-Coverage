...
class Alpha_Net_WStep(nn.Module):

    def __init__(self, action_dim, obs_dim, model_config):
        super(Alpha_Net_WStep, self).__init__()
        ...
        self.value_output = nn.Linear(hidden_sizes[-1] + 1, 1)
    ...
    def forward(self, x, steps):
        ...
        if len(steps.shape) == 1:
            steps = torch.unsqueeze(steps, axis=0)
        x = self.layers(x)
        x = torch.cat((x, steps), 1)
