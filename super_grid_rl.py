...
class SuperGridRL(gym.Env):
    ...
    def __init__(self, config: EnvContext):
        ...
        numrobot = config["numrobot"]
        gridlen = config["gridlen"]
        gridwidth = config["gridwidth"]
        maxsteps = config["maxsteps"]
        discrete_grid_values = config["discrete_grid_values"]
        collision_penalty = config["collision_penalty"]
        sensesize = config["sensesize"]
        grid = config["grid"]
        seed = config["seed"]
        free_penalty = config["free_penalty"]
        use_scanning = config["use_scanning"]
        p_obs = config["p_obs"]
        ...
        self.action_space = Discrete(4**self._numrobot)
        self.observation_space = Box(0.0, 1.0, self.get_state().shape, dtype=np.float32)
    ...
    def step(self, action):
        ...
        return state, np.sum(reward), self.done(), {}
    ...
    def get_state(self):
        ...
        obs = np.squeeze(np.expand_dims(np.stack(arrays, axis=0), axis=0), axis=0)
